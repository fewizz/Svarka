...
public class CraftInventoryDoubleChest extends CraftInventory implements DoubleChestInventory {
    ...
    public void setContents(ItemStack[] items) {
        if (getBukkitInventory().getContents().length < items.length) {
            throw new IllegalArgumentException("Invalid inventory size; expected " + getBukkitInventory().getContents().length + " or less");
        }
